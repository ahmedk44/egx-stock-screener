...
def insert_trade_signal(url: str, key: str) -> int | None:
    """Insert dummy TEST.CA Scalping signal into trade_signals and return generated id as trade_id."""
    assert requests is not None
    # Live schema (probed): id (serial), ticker, strategy_type, entry_price, stop_loss, target_1/2/3, tqi_score, shariah_status, created_at
    payload: Dict[str, Any] = {
        ...
        # created_at auto-generated by DB, but we can omit
    }
    headers = {
        "apikey": key,
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            try:
                ...
                print(f"[WARN] Failed to parse trade_id from response: {exc}")
                # Fallback: query latest TEST.CA row
                try:
