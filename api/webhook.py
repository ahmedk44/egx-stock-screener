...
def _py_handler(request, *args, **kwargs):
    """Crash-proof Vercel entrypoint - always returns 200. Handles dict and object requests."""
    try:
        if isinstance(request, dict) and 'body' in request and 'callback_query' not in request:
            raw = request.get('body', {})
            if isinstance(raw, str):
                try:
                    import json as _j
                    body = _j.loads(raw) if raw.strip() else {}
                except:
                    body = {}
                fake_req = type("obj", (), {"method": request.get("method","POST"), "body": body, "headers": request.get("headers",{}), "get_json": lambda *a, **k: body, "json": lambda: body})()
                return _handler_impl(fake_req, *args, **kwargs)
        return _handler_impl(request, *args, **kwargs)
    except Exception as _e:
        try:
            print(f"[CRITICAL WRAPPER] {_e}")
        except: pass
        return {"statusCode": 200, "body": "OK"}

# Keep original names for local imports
handler = _py_handler
app = _py_handler

def _legacy_py_handler(request, *args, **kwargs):
    try:
        return _py_handler(request, *args, **kwargs)
    except Exception:
        return {"statusCode": 200, "body": "OK"}

# Vercel Python runtime - class based handler (required for 200 response)
# This class wraps the full Telegram logic via _handler_impl
...
# Preserve function handler for programmatic imports (e.g., tests)
py_handler = _py_handler
# Alias for Vercel that expects `handler` to be class; also expose `Handler`
Handler = handler
